...
namespace _EcsFsm.Systems.AI
{
    ...
    [CreateAssetMenu(menuName = "ECS/Systems/" + nameof(LeaveAttackDistance))]
    public sealed class LeaveAttackDistance : UpdateSystem 
    {
        ...
        public override void OnUpdate(float deltaTime) 
        {
            foreach (Entity entity in _characters)
            {
                ...
                if (Vector3.Distance(moveTarget.Position, position.Value) > attack.Range)
                    entity.RemoveComponent<NearToAttack>();
            }
        }
    }
}